...
localhost/doc-build-container       latest         6464f26339bc  22 seconds ago      241 MB
...
```
...
You can check the results of the automatic publication on the [container's GitHub page](https://github.com/ESMCI/doc-builder/pkgs/container/doc-builder/doc-build-container).
...
It's vastly preferable to let GitHub build and publish the new repo using the `docker-image-build-publish.yml` workflow as described above. However, you may need to publish manually if, for instance, you introduce a change that breaks `doc-builder`.
...
```shell
podman manifest rm doc-build-container-manifest 2>/dev/null
podman manifest create doc-build-container-manifest
podman build --platform linux/amd64,linux/arm64  --manifest doc-build-container-manifest .
```
...
### Tagging
You'll next need to tag the image. Lots of container instructions tell you to use the `latest` tag, and Podman may actually add that for you. However, using `latest` in `doc-builder` can lead to support headaches as users think they have the right version but actually don't. So in addition to `latest`, you'll make a new version number incremented from the [previous one](https://github.com/ESMCI/doc-builder/pkgs/container/doc-builder/doc-build-container/versions), in the `vX.Y.Z` format.

Tag the manifest with your version number like so:
```shell
podman tag doc-build-container-manifest ghcr.io/esmci/doc-builder/doc-build-container:v2.0.1
```

Push to the repo:
```shell
podman manifest push --all doc-build-container-manifest ghcr.io/esmci/doc-builder/doc-build-container:vX.Y.Z
podman manifest push --all doc-build-container-manifest ghcr.io/esmci/doc-builder/doc-build-container:latest
```

Then browse to the [container's GitHub page](https://github.com/ESMCI/doc-builder/pkgs/container/doc-builder/doc-build-container) to make sure this all worked and the image is public.
